...
Simulation creates a [`md_sim::neighbours::CellGrid`] structure. This is a grid which bins the particles based on their positions. For each particle we then create a verlet list of particles within some distance cutoff + skin. This is efficient because it is only searching the same cell or neighbouring cells. When any particle has undergone a displacement equal to skin we rebuild the cell grid and verlet lists. In moving through each particle we do not apply Newton III. In other words if particle A is the particle of interest we calculate all the forces on it but do not automatically apply the equal and opposite forces to particle B. This is handled when particle B is the particle of interest. This is slower but it allows us to parallelise the whole thing much more easily. It also means we can be selective about whether we even need to look at particle B since perhaps it is an immobile particle or follows a prescribed trajectory. We do this by prescribing all the types of interaction that are required in the config file. e.g. "interaction_ptypes": [[0,0], [0,1], [0,2]], means interactions occur between particle types 0 and 0,1,2 but 1 and 2 experience no forces from 0. If a particle type never appears on the left hand side this means that it will experience no single_particle_forces or internal forces etc.

## Running a simulation

You need to [install uv](https://docs.astral.sh/uv/getting-started/installation/) and run `uv sync` in the terminal to generate the python environment. We assume you are using a bash terminal. If you are you can then use our commands:

- run -c sim_name  this runs the simulation called sim_name. It looks for the config file in input/sim_name and the initial state file in output/sim_name. It then writes output files to output/sim_name as it runs. If you have set up graphics it will also write video files to output/sim_name. The flag -c clears everything out and starts from scratch. If you run without it it will use the latest simulation output as its starting point.
